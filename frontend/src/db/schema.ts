...
export async function runMigrations(): Promise<void> {
  ...
  if (!applied.some(m => m[0]?.version === "013")) {
    ...
    console.log(`[Migrations] 🔍 013: Parsed ${statements.length} statements SQL`);
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        if (
          ...
          console.warn(`[Migrations] ⚠️  013: Statement ya aplicado, continuando: ${errMsg}`);
          skipped++;
        } else {
          throw new Error(`Migración 013 falló en statement ${i + 1}: ${errMsg}`);
        }
      }
    }
    ...
    console.log(`[Migrations] ✅ 013 Resumen: ${executed} ejecutados, ${skipped} saltados`);
  ...
  if (!applied.some(m => m[0]?.version === "015")) {
    ...
    console.log(`[Migrations] 🔍 015: Parsed ${statements.length} statements SQL`);
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        if (
          ...
          console.warn(`[Migrations] ⚠️  015: Statement ya aplicado, continuando: ${errMsg}`);
          skipped++;
        } else {
          throw new Error(`Migración 015 falló en statement ${i + 1}: ${errMsg}`);
        }
      }
    }
    ...
    console.log(`[Migrations] ✅ 015 Resumen: ${executed} ejecutados, ${skipped} saltados`);
