...
// BUILD_HASH: 1788495805
  ...
  /**
   * Crea un pedido y todos sus items en UNA SOLA transacción atómica.
   * Esto previene bloqueos de base de datos (database is locked) con el SyncEngine
   * al evitar múltiples aperturas/cierres de transacciones consecutivas.
   */
  static async createWithItems(
    ...
    await localDb.transaction(async (db) => {
      ...
      // 4. Encolar evento de sincronización (con los items ya incluidos conceptualmente, 
      // aunque el payload de sync se puede actualizar, por ahora encolamos el order base)
      await SyncQueueRepository.enqueueWithDb(db as any, {
        company_id: payload.company_id,
        branch_id: payload.branch_id,
        entity_type: 'order',
        entity_local_uuid: local_uuid,
    ...
    // 5. Marcar mesa como occupied (FUERA de la transacción)
    if (payload.table_id) {
      try {
        // Asumimos que localTablesService está importado o disponible
        // Si no, podemos omitirlo aquí y dejar que el SyncEngine lo maneje, 
        // pero intentaremos llamarlo si existe.
        // await localTablesService.markOccupied(payload.table_id, local_uuid, payload.company_id, payload.branch_id);
      } catch (err) {
